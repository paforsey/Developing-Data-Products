RiskScore <- function(LoanAmt, Wholesale, Profile){
    ...
    RiskScore = ((ScoreMax - ScoreMin) / (LTVMax - LTVMin) * LTV) + (ScoreMax - 
        (LTVMax * ((ScoreMax - ScoreMin) / (LTVMax - LTVMin))))
...
shinyServer(
    function(input, output){
        ...
        output$RiskScore <- renderPrint({RiskScore(input$LoanAmt, 
            input$Wholesale, input$Profile)})
    }
);
